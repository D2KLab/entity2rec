...
print('Starting hyper params optimization entity2rec...')
...
lr_v = [0.01, 0.05, 0.1]
n_estimators_v = [100, 500, 1000]
max_depth_v = [3, 5, 10]
max_features_v = ["auto", None]

for lr in lr_v:

    for n_estimators in n_estimators_v:

        for max_depth in max_depth_v:

            for max_features in max_features_v:

                print('Starting to fit the model...')

                print('lr: %f, n_estim: %d, max_depth: %d, max_feat: %s' % (lr, n_estimators, max_depth, max_features))

                rec.fit(x_train, y_train, qids_train,
                        x_val=x_val, y_val=y_val, qids_val=qids_val,
                        optimize=args.metric, N=args.N, lr=lr, n_estimators=n_estimators,
                        max_depth=max_depth, max_features=max_features)  # train the model
